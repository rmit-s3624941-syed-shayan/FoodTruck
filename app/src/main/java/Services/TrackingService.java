...
public class TrackingService
{
   ...
   public ArrayList<TrackingDetailModel> getTrackingInfoForTimeRange(Date date, int periodMinutes, int periodSeconds)
   {
      // we reparse file contents for latest data on every call
      parseFile(context);
      ArrayList<TrackingDetailModel> returnList = new ArrayList<>();
      for (TrackingInfo trackingInfo : trackingList)
         if (dateInRange(trackingInfo.date, date, periodMinutes, periodSeconds))
            returnList.add(infoToModel(trackingInfo));
      return returnList;
   }

   // the main method you can call periodically to get data that matches a given date period
   // date +/- period minutes/seconds to check
   public ArrayList<TrackingDetailModel> getTrackingInfoForId(int trackingId)
   {
      // we reparse file contents for latest data on every call
      parseFile(context);
      ArrayList<TrackingDetailModel> returnList = new ArrayList<>();
      for (TrackingInfo trackingInfo : trackingList)
         if (trackingInfo.trackableId == trackingId)
            returnList.add(infoToModel(trackingInfo));
      return returnList;
   }

   public ArrayList<TrackingDetailModel> getAllTrackings()
   {
      // we reparse file contents for latest data on every call
      parseFile(context);
      ArrayList<TrackingDetailModel> returnList = new ArrayList<>();
   ...
   public TrackingDetailModel infoToModel(TrackingInfo tI)
   {
      tI.toString();
      TrackingDetailModel returnModel = new TrackingDetailModel(tI.date, tI.trackableId, tI.stopTime, tI.latitude, tI.longitude);
